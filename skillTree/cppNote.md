...
- __必须初始化__   默认状态下，const对象仅在文件内有效，想在文件之间共享const对象，必须在前面添加extern关键字
 ...
 - 函数重载
 - 函数名字相同，形参列表不同（__返回类型不影响重载，只有返回类型不同会声明错误__）（main函数不能重载）
 - __声明函数是可以不给形参起名字__(p207)
 
 - const形参与重载
 - 顶层const被忽略
 - 非常量可以转化为常量，传递非常量参数时编译器优先选择非常量形参的版本。（p208）
 
 - 重载与作用域
 - 在作用域内如果可以找到名字，则忽略作用域外的重载（在不同的作用域中无法重载函数名）（名字查找发生在类型检查之前）（p210）
 
 - 默认实参，写在函数形参表里
 - 使用时，只能忽略尾部的实参（p212）
 - 声明默认实参时，一个作用域里只能声明一次，试图重新声明（修改）会发生重复声明错误
 - 试图修改默认实参，可以用表达式作为实参（p213）
 
 - 内联函数和constexpr函数
 - 使用内联函数可以避免函数调用的开销，
 - constexpr函数隐式地指定为内联函数
 
 - 调试帮助
 - assert预处理宏 
 - assert(expr);  //使用一个表达式作为条件
 - 表达式为假（0），终止程序运行并输出信息。如果表达式为真（非0），什么也不做（p215）
 - NDEBUG预处理变量（p216）
 ...
 ###chap 7
 - class 类
 - 基本思想：数据抽象（用户能执行的操作）和封装（类的数据成员，负责接口实现的函数体，类所需的私有函数）
 - 抽象数据类型
 
 - this指针：隐式参数，用来访问调用成员函数的对象。
 - 是一个常量指针，不允许改变指向
 
 - const成员函数
 - 参数列表之后+const，const的作用是修改隐式this指针的类型
 - 默认：this是指向类类型非常量版本的常量指针：不能绑定到常量对象，常量对象不能调用普通的成员函数
 - 将不会修改this所指对象的函数声明为常量成员函数，可以让常量对象调用，提高函数灵活性
 - 常量对象，常量对象的引用或指针只能调用常量成员函数
 
 - 定义一个返回this对象的函数（p233）
 - __函数的返回类型是引用__，return *this;  //返回this所指对象
 - 
